...
| Tenancy and RLS | Added `20260630090000_pass2_foundations.sql` with firm, fund, membership and pipeline tables, RLS enabled and forced, broad synthetic policies dropped, owner/contributor role helpers, deal-to-firm consistency checks and deterministic policy tests. |
...
| Parsing and extraction | Added deterministic segment parser, source-bound citation-span verification and candidate materialisation into existing `Fact` records while preserving same-key conflicts. |
| Retrieval and grounding | Added hybrid retrieval fusion helper with firm and deal filters, plus grounded-answer validation that abstains when cited segments are absent or exact claim text is not present. |
| Evals | Added Project Nova fixtures, including valid XLSX workbooks, gold labels, `scripts/eval.ts`, `npm run eval` and persisted latest offline report. |
...
| `npx vitest run lib/security/tenant-policy.test.ts lib/citations.test.ts lib/generate/grounding.test.ts lib/llm/client.test.ts scripts/eval.test.ts lib/ingest/source-adapter.test.ts lib/parse/segments.test.ts lib/extract/materialise.test.ts` | Passed after review fixes: 8 files, 19 tests. Covers deal-to-firm RLS checks, owner-only membership writes, source-bound citations, exact-claim grounding, mandatory LLM tenant scope, fixture presence and supersession direction. |
| `npm run eval` | Passed. Fixture files present, Overall score 61, source coverage 46, ARR conflict preserved, deterministic abstention true, cost USD 0, p50 latency 0 ms, p95 latency 0 ms. |
| `npm run eval -- --live` | Failed closed by design: `TODO(pass2): live model eval transport is not implemented. Run offline eval or wire the provider transport first.` This avoids fake live-model cost and latency metrics. |
| `npm run validate` | Passed. `validate-data` validated 9 YAML files. `lint:copy` passed across 36 files. `typecheck` passed. `next lint` passed with the two pre-existing accepted `<img>` warnings. `test` passed: 21 files, 39 tests. |
...
| `npx vercel --yes` | Passed before the review-fix commit. Created and deployed Vercel project `dealstate-pass2`, deployment `dpl_Hff5QyuWBL3N5sXQQaRYzVN9AuUR`, ready state `READY`. Remote build ran `npm ci`, `npm run build`, validation and 33 tests. A fresh deployment is needed for the latest 39-test commit. |
...
| Grounding exact claim support | 100% for offline supported claims | 100% in grounded-answer validation by exact claim text containment |
...
- `needs-secret` - Gmail OAuth client id, client secret, redirect URI and token encryption key. Needed before live mailbox sync can be implemented and tested. Current adapter still fails closed after credential checks with `TODO(pass2): Implement Gmail history sync once OAuth approval and mailbox credentials are available.`
- `needs-secret` - Model API key plus `MODEL_PROVIDER`, `MODEL_ID` and `EMBEDDING_MODEL_ID`. Needed before live structured extraction, embeddings, grounded generation and `npm run eval -- --live` can be implemented.
...
The live eval command currently fails closed until the provider transport is implemented.
